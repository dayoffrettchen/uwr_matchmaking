...
export default async function RankingPage() {
  ...
  return <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-6 px-4 py-8"><AppNavigation role={user.role} locale={locale} /><div><h1 className="text-2xl font-bold">Ranking</h1><p className="text-muted-foreground">Positionsbezogene Ratings nach Torwart, Verteidiger und Stürmer.</p></div>{data.map(([position, rows]) => <section key={position} className="grid gap-3"><h2 className="text-xl font-semibold">{POSITION_LABELS[position]}</h2><RankingTable rows={rows} /></section>)}</main>
}
