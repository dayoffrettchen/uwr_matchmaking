...
type RosterPlayer = { signupId: number; name: string; team: number | null; source: string; assignedPosition?: string | null; lineupType?: string | null }
...
function TeamColumn({ label, players, variant }: { label: string; players: RosterPlayer[]; variant: "primary" | "accent" }) {
  ...
  return <div className="rounded-lg border bg-card"><div className={`flex items-center justify-between rounded-t-lg px-4 py-2 ${variant === "primary" ? "bg-primary text-primary-foreground" : "bg-accent text-accent-foreground"}`}><span className="font-semibold">{label}</span><Badge variant="secondary">{active} aktiv · {subs} Wechsel</Badge></div><div className="grid gap-3 p-3">{PLAYER_POSITIONS.map((position) => <section key={position}><h3 className="text-sm font-semibold text-muted-foreground">{POSITION_LABELS[position]}</h3><ul className="mt-1 divide-y rounded-md border">{players.filter((p) => p.assignedPosition === position).map((p) => <li key={p.signupId} className="flex items-center justify-between px-3 py-2 text-sm"><span>{p.name}</span>{p.lineupType === "substitute" && <Badge variant="secondary">Wechsel</Badge>}</li>)}{players.filter((p) => p.assignedPosition === position).length === 0 && <li className="px-3 py-2 text-sm text-muted-foreground">Leer</li>}</ul></section>)}</div></div>
}
