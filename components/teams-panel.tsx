...
type RosterPlayer = { signupId: number; name: string; team: number | null; source: string; assignedPosition?: string | null; lineupType?: string | null; assignedRating?: number | null; rotationGroupId?: number | null; rotationGroupType?: string | null; rotationOrder?: number | null; startsInWater?: boolean | null }
...
function TeamColumn({ label, players, variant, canManage }: { label: string; players: RosterPlayer[]; variant: "primary" | "accent"; canManage: boolean }) {
  ...
  return <div className="rounded-lg border bg-card"><div className={`flex items-center justify-between rounded-t-lg px-4 py-2 ${variant === "primary" ? "bg-primary text-primary-foreground" : "bg-accent text-accent-foreground"}`}><span className="font-semibold">{label}</span><div className="flex flex-wrap justify-end gap-2"><Badge variant="secondary">{active} im Wasser · {subs} draußen</Badge>{averageRating !== null && <Badge variant="secondary">Ø MMR {averageRating}</Badge>}</div></div><div className="grid gap-3 p-3">{PLAYER_POSITIONS.map((position) => <PositionGroups key={position} position={position} players={players.filter((p) => p.assignedPosition === position)} canManage={canManage} />)}</div></div>
}

function PositionGroups({ position, players, canManage }: { position: PlayerPosition; players: RosterPlayer[]; canManage: boolean }) {
  ...
  return <section><h3 className="text-sm font-semibold text-muted-foreground">{POSITION_LABELS[position]}</h3><div className="mt-1 grid gap-2">{players.length === 0 && <div className="rounded-md border px-3 py-2 text-sm text-muted-foreground">Leer</div>}{Array.from(grouped.entries()).map(([groupId, members]) => <RotationGroupCard key={groupId} groupId={groupId} members={members.sort((a, b) => (a.rotationOrder ?? 0) - (b.rotationOrder ?? 0))} canManage={canManage} />)}</div></section>
}

function RotationGroupCard({ groupId, members, canManage }: { groupId: number; members: RosterPlayer[]; canManage: boolean }) {
  ...
  return <div className="rounded-md border px-3 py-2 text-sm"><div className="flex items-center justify-between gap-2"><span className="font-medium">{label} {groupId}</span><Badge variant="secondary">Start: {starters.map((p) => p.name).join(" + ") || "—"}</Badge></div>{waiting.length > 0 && <p className="mt-1 text-muted-foreground">Draußen: {waiting.map((p) => p.name).join(" + ")}</p>}<ol className="mt-2 grid gap-1">{members.map((p) => <li key={p.signupId} className="flex items-center justify-between gap-3"><span>{p.rotationOrder}. {p.name}</span><span className="flex flex-wrap justify-end gap-2">{p.startsInWater ? <Badge variant="outline">im Wasser</Badge> : <Badge variant="secondary">draußen</Badge>}{canManage && typeof p.assignedRating === "number" && <Badge variant="outline">MMR {p.assignedRating}</Badge>}</span></li>)}</ol>{type === "pair" && <p className="mt-2 text-muted-foreground">Wechsel: {members.map((p) => p.name).join(" ↔ ")}</p>}{type === "triple" && <p className="mt-2 text-muted-foreground">Rotation: {members.map((p, index) => `${p.name} rein → ${members[(index + members.length - 2) % members.length]?.name} raus`).join(" · ")}</p>}</div>
}
